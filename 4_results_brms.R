...
# Author: Lauren Mock
# Aim: Extract results from STAN models
# Input: 
#   Models
# Outputs: 
...
fig_path <- paste0("results/figures/", dataset, "_", mod.name, "/")
...
# Load case-crossover data
# note: using dataset after trimming to get median and 95th percentile
...
# load model
load(paste0(mod.path, mod.name, ".RData"))
gc()
...
# or_x_limits <- if(mod.name == "spline_lowPM.RData"){
#   c(0.99, 1.13)
# }else{
#   c(0.975, 1.195)
# }
...
or_plot <- res_df |>
  ...
  #                                                      * PM[2.5] * " (" * OR[11] * ")"))) +
  scale_y_discrete(labels = c("temp" = expression("Exposure to heat"), 
                              "pm25" = expression("Exposure to " * PM[2.5]), 
                              "temp_pm25" = expression("Exposure to heat & " 
                                                       * PM[2.5]))) +
  theme_classic() +
  theme(strip.background = element_blank(),
...
# reri_x_limits <- if(mod.name == "spline_lowPM.RData"){
#   c(0.99, 1.13)
# }else{
#   c(0.975, 1.195)
# }
...
reri_plot <- res_df |>
  ...
  # xlim(ifelse(mod.name == "spline_lowPM.RData", 
  #             c(0, 0.04), c(-0.03, 0.07))) +
  theme_classic() +
  theme(strip.background = element_blank(),
        axis.line.y = element_blank(),
        axis.ticks.y = element_blank(),
...
ggsave(paste0(fig_path, "or_reri.pdf"), width = 8, height = 3)
...
res_tab <- res_df %>%
  mutate(model = str_remove(mod.name, ".RData"),
         or_ci = paste0(formatC(round(point.est, n_digits), format = "f", digits = n_digits),
                        ...
                        ")"),
         prob = ifelse(prob < 0.001, "<0.001", 
                       formatC(round(prob, n_digits), format = "f", digits = n_digits)))

res_tab$estimand <- rownames(res_df)

res_tab <- res_tab %>%
  select(estimand, or_ci, prob) %>%
...
if(str_detect(mod.name, "spline")){
  ...
  mult_med <- median(mult)
  mult_ci <- quantile(mult, c(0.025, 0.975))
  paste0(mult_med, " (95% CI: ", mult_ci[1], ", ", mult_ci[2], ")")
}
...
temp_pm25_grid |>
  ...
  geom_point(x = med_temp, y = med_pm25, shape = 4, size = 2, stroke = 1.5) +
  theme_minimal() +
...
# # play around with discrete scale
# temp_pm25_grid |>
#   mutate(OR11 = cut(OR11,
...
# # try 3D plot (code from Federica)
# 
# library(plotly)
# 
# z_flat <- matrix(1,
...
# plot_ly(x = ~temp_vals, 
#         y = ~pm25_vals,
#         z = ~matrix(temp_pm25_grid$OR11, nrow = length(temp_vals)),
#         type = "surface"
#       ) %>% 
#   add_surface(
#     x = ~ temp_vals, 
